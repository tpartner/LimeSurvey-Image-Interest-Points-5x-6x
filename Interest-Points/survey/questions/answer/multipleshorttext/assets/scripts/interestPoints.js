...
/***** 
    JavaScript for the Interest Points custom question theme (LS version 5.x)
		...
		if($('#question'+qID+' .interest-points-outer-wrapper .marker').length == 0) {
			...
			$(thisImage).one('load', function() {
				...
				for (i = 0; i < markersLength; i++) {
					// Insert the markers
					var markerHTML = '<div class="marker minus parked" data-sign="-" data-index="minus-'+i+'">\
					...
					$('.interest-points-outer-wrapper:eq(0)', thisQuestion).append(markerHTML);
			...
			function positionInput(thisMarker, xRelPercent, yRelPercent) {
				...
				var thisInputWrapper = $('.input-wrapper[data-index="'+thisIndex+'"]', thisQuestion);
				var rightRule = Number(thisImage.width())-(position.left+(markerWidth/devisor))+'px';
				var leftRule = 'auto';
				if(xRelPercent <= 50) {
					var rightRule = 'auto';
					var leftRule = (position.left+(markerWidth/devisor))+'px';
				}
				var bottomRule = Number($('.interest-points-outer-wrapper:eq(0)', thisQuestion).height())-(position.top+(markerHeight/devisor))+'px';
				var topRule = 'auto';
				if(yRelPercent <= 50) {
					var bottomRule = 'auto';
					var topRule = (position.top+(markerHeight/devisor))+'px';
				}
